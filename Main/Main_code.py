...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, *args):
        ...
        if self.rect.x <= LEFT - (CELL_SIZE - SPACE):
            running = False
        if self.rect.x >= LEFT + CELL_SIZE * WIDTH_IN_CELLS:
            running = False
...
class GameOverScreen:
    def __init__(self, screen, max_score):
        ...
        self.game_over_text_rect = self.game_over_text.get_rect(center=(width // 2, height // 2 - 90))
        ...
        self.restart_button = pygame.Rect(width // 2 - 150, height // 2 + 40, 300, 50)
